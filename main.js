...
const validateCredit = arr => {
    ...
    for (let i = arr.length -2; i >= 0; i = i-2) {
      if (arr[i] * 2 > 9) {
        sumOfDigits = sumOfDigits + (arr[i] * 2 - 9);
      } else {
        sumOfDigits = sumOfDigits + arr[i] *2;
      }
    };
...
let listOfInvalidCards = [];
const findInvalidCards = nestedArr => {
  for (let k = 0; k < nestedArr.length; k++) {
    if (validateCredit(nestedArr[k])===false) {
    listOfInvalidCards.push(nestedArr[k])
    }
  }
  return listOfInvalidCards;
}
...
const idInvalidCompanies = nestedArr => {
  ...
  for (let i = 0; i < nestedArr.length; i++) {
    if (nestedArr[i][0] === 3) {
      if (listOfCompanies.indexOf('Amex') === -1) {
...
console.log(idInvalidCompanies(listOfInvalidCards))
